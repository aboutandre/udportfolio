...
var makeRandomPizza = function() {
  ...
  for (var meat = 0; meat < numberOfMeats; meat++) {
    pizza = pizza + ingredientItemizer(selectRandomMeat());
  }

  for (var veggie = 0; veggie < numberOfNonMeats; veggie++) {
    pizza = pizza + ingredientItemizer(selectRandomNonMeat());
  }

  for (var cheese = 0; cheese < numberOfCheeses; cheese++) {
...
var resizePizzas = function(size) {
  ...
  //#optimized changed the slider value to a percent width
  ...
  //#optimized Resizing of pizza containers
    var allPizzas = document.querySelectorAll(".randomPizzaContainer");
    // var dx = determineDx(allPizzas, size);
    for (var i = 0; i < allPizzas.length; i++) {
      // var newwidth = (allPizzas[i].offsetWidth + dx) + 'px';
...
window.performance.mark("mark_start_generating"); // collect timing data
  ...
  //#optimized took the var 'pizzasDiv' out of the loop since it's the same always
...
function updatePositions() {
  ...
  //#optimized moved document.body.scrollTop to variable to keep it DRY
  // this was brought outside the for loop because it's too costly inside.
  var cachedTopScroll = document.body.scrollTop;
  var phaseInt = (cachedTopScroll / 1250);

  //#optimized from querySelectorAll function by getElementsByClassName
...
//#optimized Changed to requestAnimationFrame
...
//#optimized Reduced the pizzas from 200 to 30, 200 is overkill.
  for (var i = 0; i < 30; i++) {
//#optimized removed width and height from js.
